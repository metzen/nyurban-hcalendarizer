...
var team = $('.green_block > h1 > span').text();
...
chrome.storage.sync.get(preferences, function(preferences) {
  $('.payMidWrapper > div > table > tbody > tr:gt(0)').each(function() {
    ...
    // Arbitrarily attach a summary class to the last column in the table, so
    // that the "Google Calendar (by Google)" Chrome Extension will show a small
